...
| `/bid` 从线索创建 | 自动链接来源 `[[线索池/活跃线索/线索-{client}-{topic}/线索-{client}-{topic}]]` | prospect 参数已提供 |
...
| `/change` 来源为会议 | 自动链接来源 `[[03-执行/会议纪要/{date}-{type}]]` | source=meeting |
...
| 技术答疑沉淀 | 自动链接来源 `[[售前项目/{bid}/技术交流记录/{date}]]` | 无条件 |
| 竞品实体化 | 自动链接来源 `[[售前项目/{bid}/投标结果]]` | 无条件 |
| `/work-item` action=new source=meeting | 自动在专项工作文档中链接来源 `[[03-执行/会议纪要/{date}-{type}]]` | 来源会议已指定 |
