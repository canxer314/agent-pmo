...
| 场景 | 自动操作 |
|------|----------|
| `/initiate` 创建项目 | 自动在项目章程中链接 `[[客户档案/{客户}]]` |
| `/contract` action=review | 自动链接 `[[00-项目章程]]` + `[[01-合同/主合同关键条款]]` |
| `/contract` action=sign | 自动链接 `[[01-合同/主合同关键条款]]` + `[[01-合同/履行义务清单]]` |
| `/plan` 创建里程碑 | 自动在里程碑中链接 `[[00-项目章程]]` |
| `/meeting` 创建纪要 | 自动链接 `[[00-项目章程]]` |
| `/change` 创建变更 | 自动链接 `[[04-监控/预算执行表]]` 和受影响里程碑 |
| `/work-item` action=new | 自动链接 `[[03-执行/交付看板]]`（在看板同步 TMP 行） |
| `/work-item` source=meeting | 自动链接 `[[03-执行/会议纪要/{日期}-{类型}]]`（来源会议） |
